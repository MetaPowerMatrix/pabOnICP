...
use metapower_framework::{ensure_directory_exists, AI_MATRIX_DIR, BATTERY_GRPC_REST_SERVER, BATTERY_GRPC_SERVER_PORT_START, LLMCHAT_GRPC_REST_SERVER, OFFICIAL_PATO, TICK, XFILES_LOCAL_DIR, XFILES_SERVER};
use metapower_framework::{
    dao::sqlite::MetapowerSqlite3, get_now_secs, log, read_and_writeback_json_file, ChatMessage, AI_AGENT_DIR, AI_PATO_DIR
};
use sha1::Digest;
use tokio::time::sleep;
use std::io::{BufRead, BufReader, Read};
use crate::{AirdropRequest, AllPatosResponse, ChangeBalanceRequest, EmptyRequest, FollowKolRequest, InjectHumanVoiceRequest, KolListResponse, KolRegistrationRequest, KolRelations, NamePros, NameRequest, NameResponse, PatoInfoResponse, PatoOfPro, PatoOfProResponse, PopulationRegistrationRequest, ProfessionalsResponse, RoomCreateRequest, RoomCreateResponse, RoomInfo, RoomListResponse, SimpleRequest, SimpleResponse, SnIdPaire, SnRequest, SnResponse, TokenRequest, TokenResponse, TopicChatHisResponse, TopicChatRequest, UserActiveRequest};
...
impl MetaPowerMatrixAgentService {
    ...
    pub async fn request_airdrop(&self, request: AirdropRequest) -> std::result::Result<SimpleResponse, Error> {
        ...
        let response = SimpleResponse { success, message: String::default() };

        Ok(response)
    }

    pub async fn request_population_registration( &self, request: PopulationRegistrationRequest) -> std::result::Result<SimpleResponse, Error> {
        let mut success = false;
        ...
        let response = SimpleResponse { success, message: sn.to_string() };

        Ok(response)
    }
    
    pub fn request_pato_info(&self, request: SimpleRequest,) -> std::result::Result<PatoInfoResponse, Error> {
        let id = request.id.clone();
        ...
        if let Ok(mut file) = OpenOptions::new().read(true).open(format!("{}/{}/db/tags.json", AI_PATO_DIR, id))
        {
        ...
        match MetapowerSqlite3::query_db(&select_id_table, vec!["id", "name", "registered_at", "sn"]){
            Ok(records) => {
                if !records.is_empty(){
        ...
        match MetapowerSqlite3::query_db(&select_balance_table, vec!["amount"]){
            Ok(records) => {
                if !records.is_empty() {
                    for record in records{
                        pato_info.balance = record.get("amount").unwrap().parse::<f32>().unwrap_or_default();
                    }
                }
            },
            Err(e) => {
    ...
    pub fn request_professionals(&self, _request: EmptyRequest,) -> std::result::Result<PatoOfProResponse, Error> {
            ...
            Ok(records) => {
                if !records.is_empty() {
                    for record in records{
                        ...
                        let subject = record.get("subject").unwrap().to_string();
                        pato_pro.entry(id).and_modify(|p| p.push(subject.clone())).or_insert(vec![]);
                    }
                }
            },
            Err(e) => {
        ...
        for pro in  pato_pro.iter(){
            ...
            pros.push(i);
            pros.sort_by(|a, b|{
                b.subjects.len().cmp(&a.subjects.len())
            });
        }
    ...
    pub fn request_pato_sn(&self,request: SnRequest) -> std::result::Result<SnResponse, Error> {
        ...
        let select_id_table = format!("select id, sn from pato where id in (\"{}\")", ids.join("\",\""));
        // println!("select_id_table sql: {}", select_id_table);
        ...
        match MetapowerSqlite3::query_db(&select_id_table, vec!["id", "sn"]){
            Ok(records) => {
                for record in records{
                    let id = record.get("id").unwrap().to_string();
                    let sn = record.get("sn").unwrap().parse::<i64>().unwrap().to_string();
    ...
    pub fn request_for_all_patos(&self, _request: EmptyRequest,) -> std::result::Result<AllPatosResponse, Error> {
    ...
    pub fn request_add_balance(&self,request: ChangeBalanceRequest,) -> std::result::Result<SimpleResponse, Error> {
        let amount = request.amount;
        let id = request.id.clone();
        let mut success = false;

        let add_balance = format!("UPDATE balance SET amount = amount + {} WHERE id = \"{}\"", amount, id);
    ...
    pub async fn request_minus_balance(&self,request: ChangeBalanceRequest,) -> std::result::Result<SimpleResponse, Error> {
        let amount = request.amount;
        let id = request.id.clone();
        let mut success = false;
    ...
    pub async fn request_deposit(&self,request: ChangeBalanceRequest,) -> std::result::Result<SimpleResponse, Error> {
    ...
    //TO-DO-IC-Storage
    pub fn request_inject_human_voice(&self,request: InjectHumanVoiceRequest,) -> std::result::Result<EmptyRequest, Error> {
        let roles = request.roles.clone();
        let session = request.session.clone();

        for role in roles{
            let active_session_path = format!(
                "{}/{}/live/{}",
                AI_PATO_DIR,
                role,
                session
    ...
    pub async fn request_pato_auth_token(&self,request: SimpleRequest,) -> std::result::Result<SimpleResponse, Error> {
        ...
        Ok(SimpleResponse { success: true, message: token })
    }
    
    pub fn query_pato_auth_token(&self,request: TokenRequest,) -> std::result::Result<TokenResponse, Error> {
        let select_pro_table = format!("select * from auth_token where token = \"{}\" order by registered_at", request.token);
    ...
    pub async fn request_pato_knowledges(&self,request: SimpleRequest,) -> std::result::Result<ProfessionalsResponse, Error> {
        let mut response = ProfessionalsResponse { professionals: vec![] };

        let select_pro_table = format!("select knowledge from professionals where id = \"{}\"", request.id);
    ...
    pub async fn request_topic_chat(&self, request: TopicChatRequest,) -> std::result::Result<EmptyRequest, Error> {
                ...
                Ok(records) => {
                    if !records.is_empty() {
                        ...
                        for record in records{
                            ...
                            // let id = record.get("id").unwrap().to_string();
                            let sn = record.get("sn").unwrap().to_string().parse::<i64>().unwrap_or(-1);
                            if sn > 0 {
                                ...
                                curr_input.push(chat_messages.last().unwrap_or(&"".to_owned()).to_owned()); //5
                                ...
                                if let Ok(mut client) = MetaPowerMatrixBatterySvcClient::connect(battery_address).await {
                                    let req = tonic::Request::new(
                                        MessageRequest { message: String::default(), subject: String::default(), prompt }
                                    );
                        ...
                        let saved_topic_message_file = format!("{}/{}/db/topic/{}.json", AI_AGENT_DIR, agent_id, saved_file_name);
                        if let Ok(mut sig_file) = OpenOptions::new().create(true).write(true).truncate(true).open(saved_topic_message_file){
                            let _ = sig_file.write_all(serde_json::to_string(&chat_messages).unwrap_or_default().as_bytes());
                        }
                    }
                },
                Err(e) => {
    ...
    pub fn request_topic_chat_history(&self,request: TopicChatRequest,) -> std::result::Result<TopicChatHisResponse, Error> {
        let town = request.town.clone();
        let topic = request.topic.clone();
        ...
        hasher.update(&town);
        let saved_file_name = format!("{:x}", hasher.finalize());
        let saved_topic_message_file = format!("{}/{}/db/topic/{}.json", AI_AGENT_DIR, self.id, saved_file_name);
        ...
        Ok(TopicChatHisResponse { history: chat_messages })
    }
    
    pub async fn request_create_room(&self,request: RoomCreateRequest,) -> std::result::Result<RoomCreateResponse, Error> {
        ...
        if let Ok(mut client) = ChatSvcClient::connect(LLMCHAT_GRPC_REST_SERVER).await {
            let prompt = format!("draw a picture according to the description below: {}", description);
            ...
            match client.gen_image_with_prompt(image_request).await {
                Ok(answer) => {
                    ...
                    match download_image(&image_url, &saved_local_file).await {
                        Ok(_) => {
                            let game_room_path = format!("{}/{}/db/game_room/{}", AI_PATO_DIR, owner, room_id);
                            let _ = ensure_directory_exists(&game_room_path);
                            if let Ok(mut file) = OpenOptions::new().create(true).append(true).open(format!("{}/scene.txt", game_room_path)){
        ...
        match MetapowerSqlite3::new().insert_record(new_game_room, &[&owner, &room_id, &title, &description, &xfiles_link, &town])
        {
            Ok(_) => {
        ...
        let game_room_path = format!("{}/{}/{}/db/game_room/{}", AI_AGENT_DIR, self.id.clone(), owner, room_id);
        let _ = ensure_directory_exists(&game_room_path);
        if let Ok(mut file) = OpenOptions::new().create(true).write(true).truncate(true).open(format!("{}/name.txt", game_room_path)){
            let _ = file.write_all(title.as_bytes());            
        }

        Ok(RoomCreateResponse { room_id, cover: xfiles_link })
    }
    
    pub fn request_game_room_list(&self,request: SimpleRequest,) -> std::result::Result<RoomListResponse, Error> {
        let mut response = RoomListResponse { rooms: vec![] };

        let select_room_table = format!("select owner, room_id, title, description, cover from game_room where town = \"{}\"", request.id.clone());

        match MetapowerSqlite3::query_db(&select_room_table, vec!["owner", "room_id", "title", "description", "cover"]){
            Ok(records) => {
                if !records.is_empty() {
                    for record in records{
    ...
    pub fn log_user_active(&self,request: UserActiveRequest,) -> std::result::Result<EmptyRequest, Error> {
    ...
    pub async fn request_pato_name_and_pro(&self,request: NameRequest) -> std::result::Result<NameResponse, Error> {
        let mut response = NameResponse { name_pros: vec![] };
        let ids = request.id.clone();
        let select_name_table = format!("select id, name from pato where id in (\"{}\")", ids.join("\",\""));
        ...
        match MetapowerSqlite3::query_db(&select_name_table, vec!["id", "name"]){
            Ok(records) => {
                for record in records{
                    ...
                    let name = record.get("name").unwrap().to_string();
                    let select_pro_table = format!("select subject from professionals where id = \"{}\"", id);
                    println!("select_pro_table sql: {}", select_pro_table);
                    let mut pros = vec![];
                    match MetapowerSqlite3::query_db(&select_pro_table, vec!["subject"]){
                        Ok(records) => {
                            for record in records{
                        ...
                        Err(e) => {
                            println!("Error: {}", e);
                            // return Err(tonic::Status::not_found("professionals subject not found"));
                        }
                    }
                    let mut set = HashSet::new();
                    let mut result = Vec::new();
                    ...
                    let data = NamePros { id, name, pros: result  };
    ...
    pub async fn request_pato_by_name(&self,request: SimpleRequest,) -> std::result::Result<NameResponse, Error> {
            ...
            Ok(records) => {
                if !records.is_empty() {
                    for record in records{
                        let id = record.get("id").unwrap().to_string();
                        let sn = record.get("sn").unwrap().to_string();
                        let pato = NamePros { id: id.clone(), name: name.clone(), pros: vec![sn] };
    ...
    pub fn request_kol_registration(&self,request: KolRegistrationRequest,) -> std::result::Result<EmptyRequest, Error> {
        ...
        if let Err(e) = MetapowerSqlite3::new().insert_record(add_kol, &[&id, &OFFICIAL_PATO.to_string()]){
            println!("Error: {}", e);
        }    
    ...
    pub fn request_kol_list(&self, _request: EmptyRequest,) -> std::result::Result<KolListResponse, Error> {
            ...
            Ok(records) => {
                if !records.is_empty() {
                    for record in records{
                        ...
                        relations.entry(kol).and_modify(|f| f.push(follower.clone())).or_insert(vec![follower.clone()]);
                    }
                }
            },
            Err(e) => {
        ...
        for (key, value) in relations.iter(){
            ...
            response.relations.push(KolRelations { id: key.clone(), follower: value.clone(), name });
        }
        // log!("response: {:?}", response.relations);
        Ok(response)
    }
    
    pub fn request_add_kol_follower(&self, request: FollowKolRequest,) -> std::result::Result<EmptyRequest, Error> {
    ...
    pub async fn request_marriage_registration(&self,request: KolRegistrationRequest,) -> std::result::Result<EmptyRequest, Error> {
    ...
    pub async fn request_add_marriage_follower(&self,request: FollowKolRequest,) -> std::result::Result<EmptyRequest, Error> {
        let follower = request.follower.clone();
        let id = request.id.clone();
        let add_kol = "INSERT INTO Marriage (id, follower) VALUES (?1, ?2)";
        if let Err(e) = MetapowerSqlite3::new().insert_record(add_kol, &[&id, &follower]){
    ...
    pub async fn request_marriage_list(&self, _request: EmptyRequest,) -> std::result::Result<KolListResponse, Error> {
        let mut response = KolListResponse { relations: vec![] };
